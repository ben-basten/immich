    ...
    {#if galleryInView}
      ...
      >
        <button on:click={() => memoryWrapper.scrollIntoView({ behavior: 'smooth' })} disabled={!galleryInView}>
          <CircleIconButton title="Hide gallery" icon={mdiChevronUp} color={'light'} forceDark />
        </button>
      </div>
    {/if}
    <!-- Viewer -->
    ...
    <section class="bg-immich-dark-gray m-4">
      ...
      >
        <button on:click={() => memoryGallery.scrollIntoView({ behavior: 'smooth' })}>
          <CircleIconButton title="Show gallery" icon={mdiChevronDown} color="light" forceDark />
        </button>
      </div>
